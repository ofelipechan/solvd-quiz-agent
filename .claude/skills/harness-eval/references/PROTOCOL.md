...
- Discover manifests that exist across stacks (presence-based, no assumed runtime):
  ...
  - PHP: `composer.json`, `artisan`, Symfony `bin/<console>`
...
- Skill-relative `references/` may resolve under another skill named in the same surface (e.g. “load the `dev` skill and read `references/<file>.md`”).
