...
Table columns EXACTLY (merge parser depends on it; do not add a Path column; never use a bare `|` inside a cell — write `a / b` instead):
| ID | Overall | Keep-core | Slim | Overlap | Evidence | Confidence |
```
