...
The document is intentionally stack-agnostic. Examples use TypeScript, but every principle applies to any language or runner.
...
| Component | `<Name/>` | `describe("<LoginForm/>")` |
| Hook | `useName()` | `describe("useSession()")` |
| Route / endpoint | `METHOD /path` | `describe("POST /api/auth/login")` |
...
| --- | --- | --- | --- | --- |
| **E2E** | catastrophic regression detection on stable core flows | everything (browser, server, disposable seeded DB) | third-party services only | 5–10 total |
...
| `@e2e` | one of the 5–10 stable core happy paths |
...
Is this a regression from production?
  -> lowest sufficient level (unit > integration > e2e), see § 9
```
...
| Database | real test database in integration tests; injected fake repository in unit tests |
| HTTP APIs, third-party SDKs | stub at the network / SDK boundary |
| LLMs and other non-deterministic services | stub with a fixed, minimal response; never call the real model in the suite |
...
5. **Run the tests.** Unit + integration for the touched scope; E2E when a core flow was touched. Everything green before the work is considered done. Fix failures you caused before continuing — **no deferred test fixes**.
...
| `Then the response is 200 with 2 entries` | `Then the response succeeds with 2 entries` |
| `Then the response is 401` | `Then the request is rejected as unauthenticated` |
| `Then the response is 400 with a zod issue on "topic"` | `Then the submission is rejected because the topic is missing` |
| `When the client POSTs /api/auth/login with the admin email and password` | `When the user signs in with the admin email and password` |
...
| `Then the span has attribute langfuse.user.id = "u_123"` | `Then the trace is attributed to the signed-in user` |
...
- Styling / visual appearance in unit tests
...
1. Reproduce with a failing test at the **lowest sufficient level** (unit > integration > e2e).
