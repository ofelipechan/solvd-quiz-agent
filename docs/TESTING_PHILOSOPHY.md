...
Is this a regression from production?
  -> lowest sufficient level (unit > integration), see § 9
```
...
5. **Run the tests.** Unit + integration for the touched scope. Everything green before the work is considered done. Fix failures you caused before continuing — **no deferred test fixes**.
...
1. Reproduce with a failing test at the **lowest sufficient level** (unit > integration).
